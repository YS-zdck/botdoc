...
由于小程序没有用于跟平台接口进行后台鉴权使用的用户票据。所以开发者在小程序前端鉴权会后，如果还需要将鉴权结果传递给自己的后台，做后台鉴权，这个时候在后台中需要基于 session key 针对 rawData 进行签名验证来保证数据没有被篡改过。
...
**（解密前）**
`res` 得到的是加密之后的数据，需要开发者解密，解密方案参考: [开放数据校验与解密](https://q.qq.com/wiki/develop/game/API/open_port/OpenPort.html#_2-%E5%BC%80%E6%94%BE%E6%95%B0%E6%8D%AE%E6%A0%A1%E9%AA%8C%E4%B8%8E%E8%A7%A3%E5%AF%86)
